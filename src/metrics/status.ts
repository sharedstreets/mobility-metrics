...
import {
  StatusChangeEvent,
  StatusEventError,
  StatusEventErrorType
} from "../data/status_changes";
import {
  GenericPeriodicMetric,
  FractionalCount,
  GenericMetricAggregator,
  GenericMetric
} from "./generic_aggregator";
...
export class StatusMetric extends GenericMetric {
  ...
  constructor(statusEvent: StatusChangeEvent) {
    ...
    if (!this.statusEvent.newStatus.event_time)
      throw new StatusEventError(
        StatusEventErrorType.INCOMPLETE,
        "Incomplete status data."
      );

    if (
      !this.statusEvent.initialStatus.event_time &&
      this.statusEvent.newStatus.event_time
    ) {
      this.statusType = StatusMetricType.INITIAL_STATUS;
    } else {
      if (
        this.statusEvent.initialStatus.event_type === EventType.AVAILABLE &&
        this.statusEvent.newStatus.event_type !== EventType.AVAILABLE
      ) {
        this.statusType = StatusMetricType.AVAILABLE;
      } else if (
        this.statusEvent.initialStatus.event_type === EventType.RESERVED &&
        this.statusEvent.newStatus.event_type !== EventType.RESERVED
      ) {
        this.statusType = StatusMetricType.IN_USE;
      } else if (
        this.statusEvent.initialStatus.event_type === EventType.UNAVAILABLE &&
        this.statusEvent.newStatus.event_type !== EventType.UNAVAILABLE
      ) {
...
export class H3AvailabilityStatusAggregator extends GenericMetricAggregator<
  StatusMetric,
  FractionalCount
> {
  constructor() {
  ...
  getGeoJson(weeks: string[], filterPeriod: string): {} {
    ...
    for (var h3index of h3sum.keys()) {
      var averageFractionalCount =
        h3sum.get(h3index).fractionalCount / h3sum.get(h3index).count;
      var h3Coords = h3.h3ToGeoBoundary(h3index, true);
      var h3Feature = {
        type: "Feature",
        properties: { averageFractionalCount: averageFractionalCount },
        geometry: { type: "Polygon", coordinates: [h3Coords] }
      };
...
class ShStAvailabilityStatusAggregator extends GenericMetricAggregator<
  StatusMetric,
  FractionalCount
> {
  getMetricName(): string {
  ...
  addData(data: StatusMetric) {
    if (data.statusType === StatusMetricType.AVAILABLE) {
      ...
      var shStReferenceId =
        shStstatus.initialShStStatus.shstLocationRef.referenceId;
