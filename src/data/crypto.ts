...
export abstract class EncryptableData<T>  {
    ...
    constructor(data=null, recordSecret:string='') {
        ...
        if(typeof data === "string") {
            var decipher = crypto.createDecipher(CRYPTO_ALGORITHM, secrets.getSecret() + recordSecret)
            var decryptedText = decipher.update(data,CRYPTO_OUTPUT_ENCODING, CRYPTO_INPUT_ENCODING)
    ...
    encrypt():string {
        ...
        var cipher = crypto.createCipher(CRYPTO_ALGORITHM, secrets.getSecret() + this.getRecordSecret())
        var encryptedText = cipher.update(text, CRYPTO_INPUT_ENCODING, CRYPTO_OUTPUT_ENCODING)
