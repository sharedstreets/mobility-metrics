...
export class StatusChangeEvent {
    ...
    constructor(initialStatus:MDSStatusChange, newStatus:MDSStatusChange) {
        ...
        if(initialStatus.event_time > newStatus.event_time) {
            this.error = new StatusEventError(StatusEventErrorType.OUT_OF_ORDER,  "status changes out of order");
        }
    ...
    getPeriods():PeriodicTimestamp[] {
        return getPeriodsForTimeRange(this.initialStatus.event_time, this.newStatus.event_time)
    }
}
