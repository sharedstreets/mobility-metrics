...
export abstract class MDSStatusMap {
  ...
  async *processStatusEvents(
    data: MDSStatusChangeQuery
  ): AsyncIterableIterator<StatusChangeEvent> {
    ...
    for (var newStatus of data.getSortedResults()) {
      ...
      if (!(await this.alreadyLogged(newStatus, true))) {
        ...
        if (oldStatus) yield new StatusChangeEvent(oldStatus, newStatus);
      } else {
    ...
    if (alreadyProcessedRecords > 0)
      console.log(
        alreadyProcessedRecords + " already processed records (skipping)"
      );
  }
}
...
export class InMemoryMDSStatusMap extends MDSStatusMap {
  ...
  async alreadyLogged(
    ...
    if (this.loggedStatusEvents.has(status.getId())) return true;

    if (logStatus) this.loggedStatusEvents.set(status.getId(), true);
...
export class DiskBackedMDSStatusMap extends MDSStatusMap {
  ...
  async alreadyLogged(
    ...
    if (!exists && logStatus) await this.db.put(eventKey, true);
  ...
  async setStatus(status: MDSStatusChange): Promise<MDSStatusChange> {
    ...
    if (this.encrypt) data = status.encrypt();
    else data = JSON.stringify(status);
