...
async function pointToShStLocationRef(
  point: Feature<Point>
): Promise<SharedStreetsLocationRef> {
  var lnglat = getCoord(point);

  // request
  var url =
    "https://api.sharedstreets.io/v0.1.0/match/point/" +
    lnglat[0] +
    "," +
    lnglat[1] +
    "?&searchRadius=" +
    SHST_API_SEARCH_RADIUS +
    "&maxCandidates=1&authKey=" +
    SHST_API_KEY;
...
export class ShStStatus {
  ...
  async getLocationReference(): Promise<SharedStreetsLocationRef> {
    if (this.shstLocationRef) return this.shstLocationRef;

    this.shstLocationRef = await pointToShStLocationRef(
      this.status.event_location
    );
